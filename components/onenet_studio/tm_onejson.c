...
/* Local Variables                                                           */
/*****************************************************************************/
static const char *TAG = "tm_onejson";
...
static float64_t float32_to_float64(float32_t val)
{
    ...
    sprintf(tmp_val1, "%lg", val);
    sscanf(tmp_val1, "%lf", &tmp_val2);
    return tmp_val2;
}

static int32_t set_value_with_timestamp(cJSON *data, const char *name, cJSON *value, int64_t ts_in_ms)
{
    cJSON *sub = NULL;
...
static int32_t set_value(cJSON *data, const char *name, cJSON *value)
{
    if(cJSON_IsArray(data))
    {
        if(value)
...
int32_t tm_onejson_pack_bool_with_timestamp(void *data, const char *name, boolean val, int64_t ts_in_ms)
{
    return set_value_with_timestamp((cJSON *)data, name, cJSON_CreateBool(val), ts_in_ms);
}

int32_t tm_onejson_pack_number_with_timestamp(void *data, const char *name, float64_t val, int64_t ts_in_ms)
{
    return set_value_with_timestamp((cJSON *)data, name, cJSON_CreateNumber(val), ts_in_ms);
}

int32_t tm_onejson_pack_float32_with_timestamp(void *data, const char *name, float32_t val, int64_t ts_in_ms)
{
    return set_value_with_timestamp((cJSON *)data, name, cJSON_CreateNumber(float32_to_float64(val)), ts_in_ms);
}

int32_t tm_onejson_pack_string_with_timestamp(void *data, const char *name, char *val, int64_t ts_in_ms)
{
    return set_value_with_timestamp((cJSON *)data, name, cJSON_CreateString(val), ts_in_ms);
}

int32_t tm_onejson_pack_struct_with_timestamp(void *data, const char *name, void *val, int64_t ts_in_ms)
{
    return set_value_with_timestamp((cJSON *)data, name, (cJSON *)val, ts_in_ms);
}

int32_t tm_onejson_pack_bool(void *data, const char *name, boolean val)
{
    return set_value((cJSON *)data, name, cJSON_CreateBool(val));
}
int32_t tm_onejson_pack_number(void *data, const char *name, float64_t val)
{
    return set_value((cJSON *)data, name, cJSON_CreateNumber(val));
}

int32_t tm_onejson_pack_float32(void *data, const char *name, float32_t val)
{
    return set_value((cJSON *)data, name, cJSON_CreateNumber(float32_to_float64(val)));
}

int32_t tm_onejson_pack_string(void *data, const char *name, char *val)
{
    return set_value((cJSON *)data, name, cJSON_CreateString(val));
}

int32_t tm_onejson_pack_struct(void *data, const char *name, void *val)
{
...
void *tm_onejson_get_data_by_name(void *data, const char *name)
{
...
int32_t tm_onejson_parse_string(void *data, char **val)
{
    cJSON *obj = (cJSON *)data;
...
void *tm_onejson_pack_props_and_events(void *data, const char *product_id, const char *dev_name, void *props,
...
uint32_t tm_onejson_pack_request(uint8_t *payload, int32_t msg_id, void *params, uint8_t as_raw)
{
    ...
    sprintf(temp_id, "%d", msg_id);

    cJSON_AddStringToObject(request, "id", (const char *)temp_id);
	...
	if(!cJSON_PrintPreallocated(request, (char *)payload, FEATURE_TM_PAYLOAD_BUF_LEN, 0)){
		ESP_LOGE(TAG, "cJSON_PrintPreallocated fail");
	}
	/*   cJSON_PrintUnformatted bug
    if(NULL != (temp_str = cJSON_PrintUnformatted(request)))
    {
        strcpy((char *)payload, temp_str);
        free(temp_str);
    }
	*/
    cJSON_Delete(request);

    return strlen((const char *)payload);
}

void *tm_onejson_parse_request(uint8_t *payload, uint32_t payload_len, char *msg_id, uint8_t as_raw)
{
    cJSON *root = NULL;
    cJSON *item = NULL;
    void *params = NULL;
    ...
    {
        strcpy(msg_id, item->valuestring);
        if(as_raw)
        {
            item = cJSON_GetObjectItem(root, "params");
			
			params = malloc(FEATURE_TM_PAYLOAD_BUF_LEN);
			if(!cJSON_PrintPreallocated(item, (char *)params, FEATURE_TM_PAYLOAD_BUF_LEN, 0)){
				free(params);
				ESP_LOGE(TAG, "cJSON_PrintPreallocated fail");
			}
            // cJSON_PrintUnformatted bug
...
uint32_t tm_onejson_pack_reply(uint8_t *payload, char *msg_id, int32_t msg_code, void *data, uint8_t as_raw)
{
	...
	if(!cJSON_PrintPreallocated(reply, (char *)payload, FEATURE_TM_PAYLOAD_BUF_LEN, 0)){
		ESP_LOGE(TAG, "cJSON_PrintPreallocated fail");
	}
/*
...
void *tm_onejson_parse_reply(uint8_t *payload, uint32_t payload_len, char *msg_id, int32_t *msg_code, uint8_t as_raw)
{
    cJSON *root = NULL;
    cJSON *item = NULL;
    void *data = NULL;
    ...
    *msg_code = cJSON_GetObjectItem(root, "code")->valueint;
    ...
    if(root != NULL && item != NULL)
    {
        ...
        if(as_raw)
        {
            ...
            if(NULL != item)
            {
            	data = malloc(FEATURE_TM_PAYLOAD_BUF_LEN);
				if(!cJSON_PrintPreallocated(item, (char *)data, FEATURE_TM_PAYLOAD_BUF_LEN, 0)){
					free(data);
					ESP_LOGE(TAG, "cJSON_PrintPreallocated fail");
				}
